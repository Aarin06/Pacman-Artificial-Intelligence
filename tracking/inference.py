...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for v in variables:
      variableDomainsDict[v] = []
      if v == OBS0 or v == OBS1:
          continue
      for x in range(X_RANGE):
          for y in range(Y_RANGE):
              variableDomainsDict[v].append((x,y))
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminationVariable in eliminationOrder:
            ...
            if len(joinedFactor.unconditionedVariables()) == 1:
              continue
            fullJoint = eliminate(joinedFactor, eliminationVariable)
            currentFactorsList.append(fullJoint)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        totalSum = self.total()
        randNum = random.random()*totalSum
        tempSum = 0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            observationProbability = self.getObservationProb(observation, pacmanPosition, ghostPosition, self.getJailPosition())
            self.beliefs[ghostPosition] *= observationProbability
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in self.allPositions:
                tempKeysList = newPosDistribution.keys()
                if newPos in tempKeysList: 
                    tempProduct =  self.beliefs[oldPos] * newPosDistribution[newPos]
                    tempBeliefs[newPos] += tempProduct
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(numParticles):
            actionIndex = i%numPositions
            particle = self.legalPositions[actionIndex]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            if particle in newDistribution:
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        newParticleList = []
        for currState in self.particles:
            newPosDist = self.getPositionDistribution(gameState, currState)
